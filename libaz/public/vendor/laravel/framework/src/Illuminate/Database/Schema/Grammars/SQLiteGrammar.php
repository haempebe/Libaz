...
class SQLiteGrammar extends Grammar
{
    ...
    public function compileCreate(Blueprint $blueprint, Fluent $command)
    {
        return sprintf('%s table %s (%s%s%s)',
            ...
            (string) $this->addinfoKeys($blueprint)
        );
    }
    ...
    /**
     * Get the info key syntax for a table creation statement.
     *
     * @param  \Illuminate\Database\Schema\Blueprint  $blueprint
     * @return string|null
     */
    protected function addinfoKeys(Blueprint $blueprint)
    {
        if (! is_null($info = $this->getCommandByName($blueprint, 'info'))) {
            return ", info key ({$this->columnize($info->columns)})";
        }
    }
    ...
    public function compileRenameIndex(Blueprint $blueprint, Fluent $command, Connection $connection)
    {
        ...
        $newIndex = new Index(
            ...
            $index->isinfo(), $index->getFlags(), $index->getOptions()
        );
